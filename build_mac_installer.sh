...
if [ -d "artifacts/LayerEngine.vst3" ]; then
    ...
    codesign --sign "Developer ID Application: $APPLE_DEVELOPER_ID" --options runtime --force --deep "artifacts/LayerEngine.vst3"
    cp -R "artifacts/LayerEngine.vst3" mac_pkg/root/Library/Audio/Plug-Ins/VST3/
fi

if [ -d "artifacts/LayerEngine.component" ]; then
    # Sign the AU bundle
    codesign --sign "Developer ID Application: $APPLE_DEVELOPER_ID" --options runtime --force --deep "artifacts/LayerEngine.component"
    cp -R "artifacts/LayerEngine.component" mac_pkg/root/Library/Audio/Plug-Ins/Components/
fi

if [ -d "artifacts/LayerEngine.app" ]; then
    # Sign the App
    codesign --sign "Developer ID Application: $APPLE_DEVELOPER_ID" --options runtime --force --deep "artifacts/LayerEngine.app"
