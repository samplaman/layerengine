...
static float getEnvelopeValue(double phase, WindowShape shape) {
  ...
  case WindowShape::Hann:
    return 0.5f *
           (1.0f - std::cos(2.0 * juce::MathConstants<double>::pi * phase));
...
static float getInterpolatedSample(const juce::AudioSampleBuffer &buffer,
                                   int channel, double sampleIdx) {
  ...
  if (idx1 < 0 || idx1 >= numSamples)
    return 0.0f;

  // Boundary clamping for 4-point Hermite interpolation
  float s0 = (idx0 >= 0) ? buffer.getSample(channel, idx0)
                         : buffer.getSample(channel, idx1);
...
void GranularLayer::spawnGrain() {
  ...
  for (auto &grain : grains) {
    if (!grain.active) {
      ...
      float randSize =
          1.0f + (random.nextFloat() - 0.5f) * params.sizeRandomness;
      ...
      float totalPitchOffset =
          (noteToUse - 60) + (params.pitchBend * 2.0f) + randPitch;
      ...
      // Panning
      float grainPan = juce::jlimit(-1.0f, 1.0f,
                                    params.pan + (random.nextFloat() - 0.5f) *
                                                     params.spread);
      grain.panL =
          std::cos((grainPan + 1.0f) * 0.25f * juce::MathConstants<float>::pi);
      grain.panR =
          std::sin((grainPan + 1.0f) * 0.25f * juce::MathConstants<float>::pi);
...
void GranularLayer::processBlock(juce::AudioBuffer<float> &buffer,
                                 juce::MidiBuffer &midiMessages, float gain) {
  ...
  for (auto &grain : grains) {
    if (grain.active) {
      for (int s = 0; s < numSamples; ++s) {
        ...
        double playbackOffset = grain.isReverse
                                    ? (grain.length - grain.currentOffset)
                                    : grain.currentOffset;
        double sampleIdx = grain.startSample + (playbackOffset * grain.pitch);

        // Wrap or clamp sample index if needed, but here we just stop if out of
        // bounds
        ...
        float sampleR = (sampleBuffer.getNumChannels() > 1)
                            ? getInterpolatedSample(sampleBuffer, 1, sampleIdx)
                            : sampleL;
